...
// Tipo para la definición normalizada de tickets
export interface TicketDefinition {
  ...
  tipoCreacion: 'manual' | 'automatica' | 'exclusiva_ia';
}
...
export function loadTicketDefinitions(csvFilePath: string = path.resolve(process.cwd(), 'tickets_nogal.csv')): TicketDefinition[] {
  ...
  const definitions = records.map<TicketDefinition>((row) => {
    ...
    // Determinar tipoCreacion según la columna "Tipo de creación"
    const creationCol = (row['Tipo de creación'] || '').toLowerCase();
    let tipoCreacion: 'manual' | 'automatica' | 'exclusiva_ia' = 'manual';

    if (creationCol.includes('exclusiva')) {
      tipoCreacion = 'exclusiva_ia';
    } else if (creationCol.includes('automatica') || creationCol.includes('automática')) {
